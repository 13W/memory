...
| `tree-sitter-python` | `crates/index` | The first post-v0 language grammar (ADR-0015, T24-01); ships generated C parser tables compiled by `cc`. Pinned at **0.23** (not 0.25) for ABI compatibility: the grammar's 0.25 line is ABI **15**, which `tree-sitter 0.24` (max supported language ABI 14) refuses to load — it would silently degrade to a file-only parse; 0.23.x is ABI 14 (0.23.6 verified against the pinned core), the same reason the three v0 grammars are held at 0.23. Its only runtime dependency is `tree-sitter-language` (already in the tree, shared with `tree-sitter`), so it adds **no new transitive crate**; build-time `cc` reused. Not a dense-backend/model/network SDK. | MIT |
| `tree-sitter-bash` | `crates/index` | The second post-v0 language grammar (ADR-0015, T24-02); ships generated C parser tables compiled by `cc`. Pinned at **0.23** for ABI compatibility: `tree-sitter 0.24` supports language ABI 14 at most and refuses an ABI-15 parser — it would silently degrade to a file-only parse; 0.23.3 is ABI 14 (verified against the pinned core), the same reason every earlier grammar is held at 0.23. Its only runtime dependency is `tree-sitter-language` (already in the tree, shared with `tree-sitter`), so it adds **no new transitive crate**; build-time `cc` reused. Upstream ships no `tags.scm` for bash, so the query set is hand-authored in `bash.scm`. Not a dense-backend/model/network SDK. | MIT |
| `tree-sitter-go` | `crates/index` | The third post-v0 language grammar (ADR-0015, T24-03); ships generated C parser tables compiled by `cc`. Pinned at **0.23** for ABI compatibility: `tree-sitter 0.24` supports language ABI 14 at most and refuses an ABI-15 parser — it would silently degrade to a file-only parse; 0.23.4 is ABI 14 (verified against the pinned core), the same reason every earlier grammar is held at 0.23. Its only runtime dependency is `tree-sitter-language` (already in the tree, shared with `tree-sitter`), so it adds **no new transitive crate**; build-time `cc` reused. Not a dense-backend/model/network SDK. | MIT |
| `tree-sitter-toml-ng` | `crates/index` | The fourth post-v0 language grammar (ADR-0015, T24-04); ships generated C parser tables compiled by `cc`. The `-ng` crate is the maintained fork — the unmaintained `tree-sitter-toml` predates the ABI the pinned core needs. Pinned at **0.7**, which is ABI 14 (0.7.0 verified against the pinned core); `tree-sitter 0.24` supports language ABI 14 at most and refuses an ABI-15 parser, which would silently degrade to a file-only parse. Its only runtime dependency is `tree-sitter-language` (already in the tree, shared with `tree-sitter`), so it adds **no new transitive crate**; build-time `cc` reused. Its units are `config_section`, not `symbol` (ADR-0015 Decision 3). Not a dense-backend/model/network SDK. | MIT |
| `tree-sitter-yaml` | `crates/index` | The fifth and last post-v0 language grammar (ADR-0015, T24-05); ships generated C parser tables compiled by `cc`. Pinned at **0.7**, which is ABI 14 (0.7.2 verified against the pinned core); `tree-sitter 0.24` supports language ABI 14 at most and refuses an ABI-15 parser, which would silently degrade to a file-only parse. Its only runtime dependency is `tree-sitter-language` (already in the tree, shared with `tree-sitter`), so it adds **no new transitive crate**; build-time `cc` reused. Its units are `config_section`, not `symbol` (ADR-0015 Decision 3). Not a dense-backend/model/network SDK. | MIT |
...
| `flate2` (`default-features = false`, feature `rust_backend`) | `crates/models`; `crates/xtask` dev-only | ADR-0013 makes the ONNX Runtime an artifact of first run (spec 10 §5 `[FIXED]`), and upstream publishes that library only inside its own release archives — so the installer has to open a `.tar.gz` and, for `win32-x64`, a `.zip`. `std` has no decompressor. The two alternatives were both worse: `cargo xtask dist-ort` used to shell out to the system `tar`, which is acceptable in a manually invoked dev tool and not in product code (spec 01 §1's no-external-dependency stance; T22-15 removed it from there too), and hand-writing DEFLATE is the kind of code that fails silently on the archive nobody tested. `rust_backend` is the load-bearing part: it selects `miniz_oxide`, pure Rust, so there is no C toolchain and no system zlib per platform — the same reason ADR-0005 chose `rustls` over OpenSSL. Transitive set, all pure Rust: `miniz_oxide`, `adler2`, `simd-adler32`, `crc32fast`. The tar and zip **container** walks are this project's own (`crates/models/src/archive.rs`); only the compressed stream is `flate2`'s. | MIT OR Apache-2.0 |
...
| `sysinfo` (`default-features = false`, feature `system`) | `crates/xtask` (dev-only task runner, never shipped) | Spec 14 §2's `resources` acceptance gate needs a real daemon's idle RSS (T17-05's `release-report` subcommand); `std` exposes no process memory introspection at all. `default-features = false` + `system` keeps this to exactly `System`/`Process` (`refresh_processes`, `Process::memory`) — no `disk`/`network`/`component`(sensors)/`user` probing. Chosen over shelling out to `ps` (which would need per-OS parsing and does not exist on `win32-x64`, a real v0 target, spec 01 §2) or hand-rolled `libc::getrusage`/platform `#[cfg]` branches (macOS/Linux report `ru_maxrss` in different units, and neither covers Windows) specifically because one API measures every real release target. `xtask` is `publish = false`, dev-only, and excluded from `default-members` — this dependency never reaches a shipped binary. Native-linking transitive set on this host (macOS): `objc2-core-foundation`, `objc2-io-kit` (both Zlib OR Apache-2.0 OR MIT, IOKit/CoreFoundation process/memory queries). `Cargo.lock` additionally resolves the Windows-only branch of `sysinfo`'s own dependency tree (`windows`, `windows-core`/`-collections`/`-future`/`-implement`/`-interface`/`-numerics`/`-result`/`-strings`/`-threading`, `ntapi`, `winapi`, `winapi-i686-pc-windows-gnu`, `winapi-x86_64-pc-windows-gnu`) — all `cfg(windows)`-gated, never compiled on the macOS/Linux hosts this environment can build on, disclosed here rather than silently accepted (the same "resolved but never compiled for v0's shipped targets" pattern `notify`'s own row already documents for its `kqueue`/`wasi` branches). None are dense-backend/model/network SDKs. | MIT (sysinfo) / Zlib OR Apache-2.0 OR MIT (objc2-core-foundation, objc2-io-kit) |
| `clap` (feature `derive`) | `crates/local-rag` (the `local-rag` binary's CLI only — `local-rag-proxy`/`local-rag-hook`/`xtask` keep hand-rolled `std::env::args()`, spec 11 §6's original as-built note) | X-002 (explicit post-G17 product decision): the CLI's 17-command, multi-level surface (`repo attach`, `memory list\|approve\|reject\|edit\|retract\|merge\|evidence`, `inspect <kind> <id>`, …) had outgrown one hand-written flag-loop per subcommand in `crates/local-rag/src/cli/*.rs` — each with its own usage string and error formatting, and no `Vec<T>` primitive for `memory merge`'s repeatable `--loser <id>:<version>`. `std` has no argument parser; `clap`'s `derive` feature generates a typed `Parser`/`Subcommand` tree per module instead, matching this crate's existing typed-boundary style elsewhere. Native-linking transitive set (`cargo tree -p local-rag -e normal`, verified after `cargo update`): `clap_builder`, `clap_derive` (build-time proc-macro; reuses `proc-macro2`/`quote`/`unicode-ident` already resolved by `serde_derive`, plus a **second, newer** `syn` line — v3.0.3 alongside the existing v2.0.119 `serde_derive`/`thiserror`/`tokio-macros` pin, the same "two versions, both build-time only" disclosure `llama-cpp-sys-2`'s row already uses for `itertools`/`libloading`), `clap_lex`, `anstream`/`anstyle`/`anstyle-parse`/`anstyle-query`/`colorchoice`/`is_terminal_polyfill`/`utf8parse` (colored `--help`/error rendering, auto-disabled on a non-tty or `NO_COLOR`), `strsim` (did-you-mean suggestions on an unknown subcommand/flag), `heck` (build-time, `clap_derive`'s kebab-case flag naming), `once_cell_polyfill`. `Cargo.lock` additionally resolves `anstyle-wincon` (Windows console coloring) — `cfg(windows)`-gated, never compiled on the macOS/Linux hosts this environment can build on, same disclosure pattern as `notify`'s/`sysinfo`'s Windows-only branches above. None are dense-backend/model/network SDKs. | MIT OR Apache-2.0 (`strsim`: MIT) |
| `ratatui` (`default-features = false`, feature `crossterm`) | `crates/local-rag-tui` | ADR-0008 `[FIXED]` picks `ratatui`+`crossterm` for the TUI dashboard (T18-01); `std` has no terminal-UI widget/layout toolkit. `default-features = false` + `crossterm` selects exactly the crossterm backend (this workspace never needs `termion`/`termwiz`/`termina`) and skips the `all-widgets` default (only adds the calendar widget + its own `time`/`palette`/`approx` dependencies — none of ADR-0008's six screens is a calendar), `macros`, `layout-cache`, `palette`, `serde`. `Cargo.lock` still resolves `palette`/`palette_derive`/`palette_math`/`time`/`approx`/`time-core`/`deranged`/`num-conv`/`powerfmt`/`libm`/`num_threads`/`rustc_version`/`semver`/`by_address` (ratatui-core's own optional dependencies for the un-activated `all-widgets`/`serde` features) but none of them compile into anything this crate links — verified: `cargo tree -p local-rag-tui -i palette --target all` prints nothing. As of ratatui 0.30 the crate is workspace-split: `ratatui-core`, `ratatui-widgets`, and (via the `crossterm` feature) `ratatui-crossterm`. Native-linking transitive set actually new to this workspace (`cargo tree -p local-rag-tui -e normal --target all`, diffed against `Cargo.lock` before this task): `kasuari` (a maintained fork of the unmaintained `cassowary-rs` layout-constraint solver), `line-clipping`, `lru`, `strum`+`strum_macros` (build-time), `unicode-truncate`, `unicode-width`, `allocator-api2` (via `hashbrown`), `instability` (build-time, +`indoc`/`litrs` its own build deps). `bitflags`/`compact_str`/`hashbrown`/`itertools`/`thiserror`/`unicode-segmentation` were already resolved elsewhere in the workspace (`clap`/`tokenizers`/`ignore`, etc.) — 0 new sources for those. None are dense-backend/model/network SDKs. | MIT (ratatui, ratatui-core, ratatui-widgets, ratatui-crossterm, kasuari, line-clipping, lru, strum, strum_macros, unicode-truncate, instability); MIT OR Apache-2.0 (unicode-width, allocator-api2) |
| `crossterm` | `crates/local-rag-tui` | The terminal backend itself (raw mode, alternate screen, key/mouse/resize event polling) — `std` has no cross-platform terminal control API, and this is ADR-0008's own `[FIXED]` choice over `termion`/`termwiz` (POSIX-only) precisely because this workspace ships a real win32-x64 dist target (`dist-workspace.toml`). Declared as a direct dependency even though `ratatui-crossterm` already resolves it transitively — this workspace's convention is an explicit edge for a crate this code imports by name, not reliance on transitive unification (mirrors `serde_json`'s row above). Default features kept, unlike most rows in this table: `events` (`mio`+`signal-hook` on unix) is what the synchronous `event::poll`/`event::read` API this crate's own event loop calls is built on, `windows` (`crossterm_winapi`) is required for the win32-x64 target, `bracketed-paste`/`derive-more` are crossterm's own low-cost internal defaults. Native-linking transitive set actually new to this workspace (same verification method as `ratatui`'s row above): `document-features` (build-time, +`litrs`), `derive_more`+`derive_more-impl` (build-time, +`convert_case` used by the derive), `parking_lot`+`parking_lot_core`+`lock_api`, `rustix`+`linux-raw-sys` (unix-only), `signal-hook`, `signal-hook-mio`, `crossterm_winapi` (`cfg(windows)`-only, never compiled on the macOS/Linux hosts this environment can build on — same disclosure pattern as `notify`'s/`sysinfo`'s/`clap`'s own Windows-only branches above); `mio`/`libc` were already resolved elsewhere in the workspace (`notify`) — 0 new sources for those. Not a dense-backend/model/network SDK. | MIT |
| `tracing` | `crates/local-rag` (daemon-side emitter; the library half of every other crate stays silent) | X-004 (explicit post-G17 product decision): `local-rag serve`, run manually in a terminal, printed nothing at all — no logging subsystem existed anywhere in this workspace (spec 07 §4's own as-built note said so plainly). `std` has no structured, level-filtered event-emission facade. `tracing` itself adds **0 new external sources** — `tracing`/`tracing-core`/`tracing-attributes` were already resolved transitively via `local-rag-generate`'s `llama-cpp-2` and `local-rag-embed`'s `ort`; this is only a new direct edge for the name. Not a dense-backend/model/network SDK. | MIT (tracing, tracing-core, tracing-attributes) |
| `tracing-subscriber` (`default-features = false`, features `fmt`+`env-filter`) | `crates/local-rag` (only `main.rs::logging::init` calls `tracing_subscriber::fmt()` — no other binary or library crate in the workspace installs a subscriber) | The daemon's own stderr sink for `tracing`'s events (`RUST_LOG` > `config.daemon.log_level` > `"info"`, spec 02 §3.1's as-built note) — `std` has no such sink. `default-features = false` + exactly `fmt`+`env-filter` skips `ansi` (would corrupt the byte-exact substring assertions in `tests/serve_logging.rs`, and is useless once stderr is redirected to a pipe or file), `tracing-log` (no bridge from the `log` facade — nothing else in this workspace emits through it), `json`, and `registry`'s own optional extras. Native-linking transitive set new to this workspace (`cargo tree -p local-rag -e normal --target all`, diffed against `Cargo.lock` before this task): `matchers` (the `env-filter` feature's regex-based per-target/per-field directive matcher — verified via `cargo tree -p local-rag -e normal --target all -i regex`, its own `regex` 1.13.1 edge is the exact same normal-dependency instance `tokenizers`/`tree-sitter` already pull in, not a new one), `sharded-slab` (`fmt`'s lock-free per-span storage), `thread_local`, `lazy_static`. `regex`/`regex-automata`/`regex-syntax`/`once_cell`/`cfg-if`/`pin-project-lite` were all already resolved elsewhere in the workspace — 0 new sources for any of them. None are dense-backend/model/network SDKs. | MIT (tracing-subscriber, matchers, sharded-slab); MIT OR Apache-2.0 (thread_local, lazy_static) |
| `tracing-appender` | `crates/local-rag` (only `main.rs::logging::init`, the file half of the same subscriber `tracing-subscriber` provides the stderr half of) | X-007 (explicit post-G17 product decision): X-004's stderr-only sink is unreadable in the normal MCP setup, because `local-rag-proxy` starts the daemon with `Stdio::null()` on stderr (`crates/local-rag-proxy/src/connect.rs:76-82`) — so every line, including X-006's indexing-cycle events, was discarded. `std` can append to a file, but not rotate one: daily rotation plus a bounded file count means date formatting and a delete-oldest sweep, and this workspace has already declined to take on a date-formatting crate for its own code (D-056). This is the `tokio-rs` crate written for exactly this job, used **without** its `non_blocking`/`WorkerGuard` half (a synchronous writer cannot lose the lines just before an exit or crash — the ones most worth keeping). Transitive set new to this workspace (diffed `Cargo.lock` before/after): `crossbeam-channel` (its unused-here `non_blocking` path), `symlink` (maintains a stable symlink to the current file), `time-macros` (`time`'s own compile-time format machinery). `time`/`time-core`/`deranged`/`powerfmt`/`num-conv` were already resolved via `ratatui-widgets`, and `thiserror`/`thread_local`/`tracing-subscriber` were already direct or transitive here — 0 new sources for any of them. None are dense-backend/model/network SDKs. | MIT (tracing-appender, symlink); MIT OR Apache-2.0 (crossbeam-channel, time-macros) |
...
- Product binaries: `local-rag` (daemon + CLI), `local-rag-proxy` (stdio MCP
  proxy), `local-rag-hook` (spool writer), `local-rag-tui` (terminal dashboard
  skeleton, ADR-0008/T18-01 — real screens land in later group-18 cards).
