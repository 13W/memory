...
| O6 | Retention K / T for retired generations | boundary made explicit at T17-05 (06 §5's own as-built note): no usage-metrics telemetry exists anywhere in this codebase to derive real numbers from, and building it is a separate project, not in scope here — v0 ships the current provisional defaults (`K = 2`, `T = 168h`) unchanged; whether GA re-derives them from real telemetry is a pre-GA release-gate item, not silently dropped | GC defaults |
