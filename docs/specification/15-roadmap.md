...
| O4 | First-release language set | **RESOLVED — ADR-0001 (T04-01): TypeScript, JavaScript, Rust**; **extended post-v0 — ADR-0015 (group 24, T24-01…T24-05): + Python, Bash, Go, TOML, YAML**, one language per card, additive with no schema or identity change (a new token in the fingerprint's existing `lang=` field). The v0 answer above is unchanged: this question asked for the *first-release* set, and §2's MVP scope still describes what v0 shipped | parser scope |
