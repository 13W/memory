...
| Hit@5 / Recall@5 | 0.8367 | 0.7959 | -0.0408 |
| MRR | 0.6963 | 0.5721 | -0.1241 |
...
| index | 1560 |
| embed | 291297 |
| warm search p50 | 125.447 |
| warm search p95 | 146.591 |
...
| sc-02 | embedder | 4 | `embedder.ts` / `embedOne` |
...
| sc-08 | embedder | 2 | `embedder.ts` / `llmFilter` |
| sc-09 | embedder | — | — |
| sc-10 | embedder | 2 | `embedder.ts` / `resolveEmbedUrl` |
...
| sc-15 | indexer | 4 | `indexer/indexer.ts` / `_indexFileImpl` |
| sc-16 | indexer | 1 | `indexer/indexer.ts` / `buildEmbedContext` |
| sc-17 | indexer | 2 | `indexer/indexer.ts` / `collectFiles` |
| sc-18 | indexer | 3 | `indexer/indexer.ts` / `CodeIndexer` |
| sc-19 | parser | 1 | `indexer/parser.ts` / `parseFile` |
| sc-20 | parser | 3 | `indexer/parser.ts` / `walkTree` |
...
| sc-24 | parser | 3 | `indexer/parser.ts` / `extractImports` |
| sc-25 | types | 1 | `types.ts` / `CodeChunkPayload` |
| sc-26 | types | 1 | `types.ts` / `CodeChunkPayload` |
| sc-27 | qdrant | 1 | `qdrant.ts` / `ensureCodeChunks` |
| sc-28 | qdrant | — | — |
| sc-29 | qdrant | 2 | `qdrant.ts` / `ensureCollections` |
| sc-30 | config | — | — |
| sc-31 | storage | — | — |
| sc-32 | storage | 4 | `storage.ts` / `getReverseDeps` |
| sc-33 | storage | 4 | `storage.ts` / `getMemoryMeta` |
| sc-34 | storage | 1 | `storage.ts` / `topFilesByRevDeps` |
| sc-35 | server | — | — |
| sc-36 | server | — | — |
| sc-37 | tools | 5 | `tools/remember.ts` / `rememberTool` |
| sc-38 | tools | — | — |
