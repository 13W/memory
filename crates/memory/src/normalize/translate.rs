...
pub const TRANSLATOR_VERSION: i64 = 1;

/// Longest source text this translator accepts, in characters.
///
/// `[SPEC]`-chosen: with ~4 characters per token it leaves the prompt, the
/// source and a doubled output comfortably inside the default model's 32k
/// context, so a translation attempt never turns into a
/// [`GenError::ContextOverflow`] the caller has to classify. It is also far
/// above what recall renders per entry (1 KiB), so nothing that could plausibly
/// improve a recall result is refused.
...
pub fn validate(source: &str, response: &GenResponse) -> Result<String, TranslateError> {
    ...
    let answer: TranslationAnswer = serde_json::from_str(body)
        .map_err(|e| TranslateError::Rejected(format!("not a single {{\"en\": …}} object: {e}")))?;
    let english = answer.en;
