...
fn sha256(bytes: &[u8]) -> [u8; 32] {
    ...
    // Pad to a whole number of 64-byte blocks: 0x80, then zeros, then the
    // 64-bit big-endian message length in bits.
    let bit_len = (bytes.len() as u64).wrapping_mul(8);
    let mut msg = Vec::with_capacity(bytes.len() + 9 + 63);
...
mod tests {
    ...
    // NIST FIPS-180-4 known-answer tests: the expected digests are independent
    // ground truth, so a matching output proves the implementation (not a
    // circular self-check).
    #[test]
    ...
    fn multi_block_vector() {
        // 56 bytes forces a second block after padding (length spills past the
        // first block's 56-byte tail).
        assert_eq!(
    ...
    fn digest_is_lowercase_hex_of_len_64() {
        let hex = sha256_hex(b"local-rag");
