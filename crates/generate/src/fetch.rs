...
//!
//! Mirrors `local_rag_models::fetch` exactly (see `crates/generate`'s own
//! Cargo.toml comment for why this is duplicated rather than shared):
//! [`AssetFetcher`] exists so the installer's *policy* (atomic writes,
//! digests, resumability, the `.ok` marker) can be tested exhaustively
//! without a network. Production uses [`HttpFetcher`]; tests use
//! [`LocalFetcher`] or a loopback fixture server.
//!
//! Downloading a GGUF weight file is not gated on `data_policy`, for the same
//! reason `local_rag_models::fetch`'s own module doc gives: spec 12 §1 and
//! spec 10 §1's policy guard govern *provider selection* (sending repository
//! content out), not an explicit user command pulling public bytes in.
...
/// A fetcher that serves assets from a local directory, addressing them by
...
mod tests {
    ...
    fn a_local_fetcher_streams_a_file_by_url_tail() {
        ...
        std::fs::write(dir.join("model.gguf"), b"fake weights").expect("seed");

        let fetcher = LocalFetcher::new(&dir);
        let mut sink = Vec::new();
        let n = fetcher
            .fetch("https://example.invalid/repo/model.gguf", &mut sink)
            .expect("fetch");

        assert_eq!(n, 12);
        assert_eq!(sink, b"fake weights");
    }
    ...
    fn a_missing_local_asset_is_a_transport_error() {
        ...
        let mut sink = Vec::new();
        let err = fetcher
            .fetch("https://example.invalid/nope.gguf", &mut sink)
            .expect_err("absent");
