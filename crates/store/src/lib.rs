...
pub use cache::{
    ...
    candidate_depth, decode_vector_le, delete_embedding, delete_normalized_text,
    document_frequencies, encode_vector_le, flush_last_used, flush_last_used_embeddings,
...
pub use memory::{
    Actor, ApplyReport, ApproveCandidateOutcome, AuditEventRow, CandidateRow, CandidateState,
    CandidateTransitionError, ConsolidationWindow, CreateMemoryEntryError, CreateMemoryOp,
    EditMemoryOp, EvidenceInput, GLOBAL_SCOPE_OWNER_ID, GeneratedOp, IllegalCandidateTransition,
    IllegalMemoryTransition, IllegalRunTransition, LEASE_DURATION_MS, LEASE_RENEW_INTERVAL_MS,
    MemoryKind, MemoryOpError, MemoryOpOutcome, MemoryOpResult, MemoryState, MemoryTransitionError,
    MergeLoser, MergeMemoryOp, NewAuditEvent, NewCandidate, NewConsolidationRun, NewMemoryEntry,
    NewMemoryEvidence, ProposedOperation, ReinforceMemoryOp, RenewError, ResolveMemoryOp,
    RetractMemoryOp, ReviewError, RunOutcome, RunOutcomeError, RunState, RunTransitionError,
    RunWindow, RunnerApplyError, RunnerError, ScopeKind, SnapshotOutcome, StaleRun,
    SupersedeMemoryOp, WindowObservation, acquire_lease, apply_create, apply_edit, apply_merge,
    apply_noop, apply_reinforce, apply_resolve, apply_retract, apply_supersede, approve_candidate,
    candidate_evidence_for, candidate_state, commit_apply_run, consolidation_run_state,
    create_candidate, create_consolidation_run, create_memory_entry, edit_candidate,
    find_by_idempotency_key, insert_audit_event, insert_candidate_evidence, insert_memory_evidence,
    lease_expired, list_candidates, memory_entry_state, memory_evidence_for, open_next_run,
    pending_candidate_ages, processing_cursor, propose_candidate, read_audit_events_for_entity,
