...
pub use memory::{
    AUDIT_ENTITY_CONSOLIDATION_RUN, AbandonOutcome, Actor, ApplyReport, ApproveCandidateOutcome,
    AuditEventRow, BacklogBlocker, CANDIDATE_DEDUP_KEY_VERSION, CURRENT_NORMALIZER_VERSION,
    CandidateCountRow, CandidateDedupKey, CandidateRow, CandidateState, CandidateTransitionError,
    ClassifiedFailure, ConfirmMemoryOp, ConsolidationWindow, CreateMemoryEntryError,
    CreateMemoryOp, DeadLetteredNormalization, EditMemoryOp, EvidenceInput, FailureKind,
    GLOBAL_SCOPE_OWNER_ID, GeneratedOp, IllegalCandidateTransition, IllegalMemoryTransition,
    IllegalRunTransition, LEASE_DURATION_MS, LEASE_RENEW_INTERVAL_MS, MAX_NORMALIZATION_ATTEMPTS,
    MemoryCountRow, MemoryEntryRow, MemoryEntrySummary, MemoryKind, MemoryOpError, MemoryOpOutcome,
    MemoryOpResult, MemoryState, MemoryTransitionError, MergeLoser, MergeMemoryOp, NewAuditEvent,
    NewCandidate, NewConsolidationRun, NewMemoryEntry, NewMemoryEvidence, NormalizationBacklog,
    NormalizationCountRow, NormalizationRow, NormalizationStatus, NormalizationWrite,
    PendingNormalization, ProposeCandidateOutcome, ProposedOperation, RecallCandidate,
    ReinforceMemoryOp, RejectMemoryOp, RenewError, RepairError, ResolveMemoryOp, RetractMemoryOp,
    ReviewError, RunCountRow, RunOutcome, RunOutcomeError, RunState, RunTransitionError, RunWindow,
    RunnerApplyError, RunnerError, STUCK_RUN_ATTEMPT_THRESHOLD, STUCK_RUN_REASON_MAX_CHARS,
    ScopeKind, SessionBacklog, SnapshotOutcome, StaleRun, StuckRunRow, SupersedeMemoryOp,
    TRANSIENT_BACKOFF_BASE_MS, TRANSIENT_BACKOFF_CAP_MS, UNBOUNDED_WINDOW_CHARS,
    UnconsolidatableSession, UpsertOutcome, WindowObservation, abandon_run, acquire_lease,
    ...
    apply_resolve, apply_retract, apply_supersede, approve_candidate, candidate_dedup_key,
    candidate_evidence_for, candidate_state, canonical_key_owner, commit_apply_run,
    consolidation_run_counts, consolidation_run_state, create_candidate, create_consolidation_run,
    create_memory_entry, dead_lettered_normalizations, delete_normalization, edit_candidate,
    ...
    pending_backlog_by_session, pending_candidate_ages, pending_candidate_counts,
    processing_cursor, propose_candidate, read_audit_events_for_entity, recall_candidate_by_id,
    recall_candidates_for_scope, record_run_failure, reject_candidate, renew_lease,
    retry_parked_run, retry_run, run_once, session_idle_since, sessions_with_pending_backlog,
    stale_runs, stuck_consolidation_runs, total_pending_backlog, transient_backoff_delay_ms,
