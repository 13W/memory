...
pub use code::{
    ...
    SOURCE_ZSTD_LEVEL, SkipReason, SkipTally, SourceCompression, UnitKind, content_blob_exists,
    ...
    detect_newline_style, file_revision_id_by_content_key, generation_accounted_paths,
    generation_file_count, generation_file_occurrence_counts, generation_skip_tally,
...
pub use memory::{
    AUDIT_ENTITY_CANDIDATE, AUDIT_ENTITY_CONSOLIDATION_RUN, AUDIT_OP_FOLD_DUPLICATE,
    AbandonOutcome, Actor, ApplyReport, ApproveCandidateOutcome, AuditEventRow, BacklogBlocker,
    CANDIDATE_DEDUP_KEY_VERSION, CURRENT_NORMALIZER_VERSION, CandidateCountRow, CandidateDedupKey,
    CandidateGroup, CandidateRow, CandidateState, CandidateTransitionError, ClassifiedFailure,
    ConfirmMemoryOp, ConsolidationWindow, CreateMemoryEntryError, CreateMemoryOp,
    DeadLetteredNormalization, EditMemoryOp, EvidenceInput, FailureKind, FoldDuplicatesOutcome,
    FoldRetained, GLOBAL_SCOPE_OWNER_ID, GeneratedOp, IllegalCandidateTransition,
    IllegalMemoryTransition, IllegalRunTransition, LEASE_DURATION_MS, LEASE_RENEW_INTERVAL_MS,
    MAX_NORMALIZATION_ATTEMPTS, MemoryCountRow, MemoryEntryRow, MemoryEntrySummary, MemoryKind,
    MemoryOpError, MemoryOpOutcome, MemoryOpResult, MemoryState, MemoryTransitionError, MergeLoser,
    MergeMemoryOp, NewAuditEvent, NewCandidate, NewConsolidationRun, NewMemoryEntry,
    NewMemoryEvidence, NormalizationBacklog, NormalizationCountRow, NormalizationRow,
    NormalizationStatus, NormalizationWrite, PendingNormalization, ProposeCandidateOutcome,
    ProposedOperation, RecallCandidate, ReinforceMemoryOp, RejectMemoryOp, RenewError, RepairError,
    ...
    STUCK_RUN_ATTEMPT_THRESHOLD, STUCK_RUN_REASON_MAX_CHARS, ScopeKind, SessionBacklog,
    SnapshotOutcome, StaleRun, StuckRunRow, SupersedeMemoryOp, TRANSIENT_BACKOFF_BASE_MS,
    TRANSIENT_BACKOFF_CAP_MS, UNBOUNDED_WINDOW_CHARS, UnconsolidatableSession, UpsertOutcome,
    WindowObservation, abandon_run, acquire_lease, active_entries_for_scope,
    active_entry_with_text, all_memory_entries_with_text, apply_confirm, apply_create, apply_edit,
    apply_merge, apply_noop, apply_reinforce, apply_reject, apply_resolve, apply_retract,
    apply_supersede, approve_candidate, candidate_dedup_key, candidate_evidence_for,
    ...
    entries_needing_normalization, find_by_idempotency_key, fold_pending_duplicates,
    ...
    open_next_run, pending_backlog, pending_backlog_by_session, pending_candidate_ages,
    pending_candidate_counts, pending_candidate_groups, processing_cursor, propose_candidate,
    read_audit_events_for_entity, recall_candidate_by_id, recall_candidates_for_scope,
    record_run_failure, reject_candidate, renew_lease, retry_parked_run, retry_run, run_once,
    session_idle_since, sessions_with_pending_backlog, stale_runs, stuck_consolidation_runs,
    total_pending_backlog, transient_backoff_delay_ms, transition_candidate,
    transition_memory_entry, transition_run, unconsolidatable_sessions, upsert_normalization,
    upsert_processing_cursor,
};
pub use memory::{DedupTriageReport, TriageError, fold_all_pending_duplicates};
pub use migrate::{
