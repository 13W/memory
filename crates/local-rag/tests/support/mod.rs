...
pub async fn seed_pending_candidate(
    ...
    // proposal, so the text must be distinct per candidate for a caller that
    // wants more than one row to survive.
...
pub fn start_options(layout: StoreLayout) -> StartOptions {
    ...
    StartOptions {
        ...
        gc_poll_interval: std::time::Duration::from_secs(3600),
    }
}
