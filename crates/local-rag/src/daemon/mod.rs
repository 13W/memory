...
//! recovery; [`probe`] is the minimal, provisional liveness handshake `lock`
//! uses to tell a live prior owner apart from a dead one (PID reuse-safe).
//! [`session`]/[`jobs`] are the two live-count registries the idle-shutdown
//! gate ([`idle`]) reads. [`resume`] is the two startup catch-up passes spec
//! 02 §4.1 step 5 names. [`mode`] is the daemon's serving mode (spec 02 §6);
//! [`handshake_stub`] is what the socket answers with it before T15-02's
//! real protocol exists; [`error`] maps a migration failure into both.
...
pub use handshake_stub::serve_handshake_stub;
...
pub use probe::{Greeting, LIVENESS_PROBE_TIMEOUT_MS, LivenessOutcome, LivenessProbe};
pub use resume::{
