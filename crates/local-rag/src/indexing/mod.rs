...
mod tests {
    ...
    #[tokio::test]
    async fn indexing_a_python_file_makes_its_function_searchable_by_name() {
        // T24-01 acceptance (ADR-0015): `search_code` finds a Python function by
        // name on a fixture store — the `.py` route goes through the real
        // `tree-sitter-python` adapter (a named `symbol`, `lang=python`), not the
        ...
        assert!(
            response.results.iter().any(|r| {
                r.path.ends_with("app.py")
                    && r.unit_kind == "symbol"
                    && r.name == "parse_config"
                    && r.language == "python"
            }),
            "expected a named python symbol hit: {:?}",
            response.results
        );
    }
    ...
    #[tokio::test]
    async fn indexing_a_toml_file_makes_its_table_searchable_by_name() {
        // T24-04 acceptance (ADR-0015): `search_code` finds a TOML table by name on a
        // fixture store. Before this card `.toml` went through the universal line
        // scanner, which never treats `[dependencies]` as a section start — the file
        // was sectioned by its first keys instead. The hit is a `config_section`,
        // not a `symbol` (ADR-0015 Decision 3).
        let home = TempHome::new().expect("temp home");
        let layout = StoreLayout::new(home.join("local-rag"));
        layout.ensure().expect("ensure store tree");
        let root = home.join("repo");
        ...
        assert!(
            response.results.iter().any(|r| {
                r.path.ends_with("Cargo.toml")
                    && r.unit_kind == "config_section"
                    && r.name == "dependencies"
                    && r.language == "toml"
            }),
            "expected a named toml config_section hit: {:?}",
            response.results
        );
    }

    #[tokio::test]
    async fn indexing_a_yaml_file_makes_its_section_searchable_by_name() {
        // T24-05 acceptance (ADR-0015): `search_code` finds a YAML section by name
        // on a fixture store, and the section belongs to its own document. Before
        // this card the universal line scanner merged the two documents into one
        // flat key list. The hit is a `config_section`, not a `symbol`.
        let home = TempHome::new().expect("temp home");
        let layout = StoreLayout::new(home.join("local-rag"));
        layout.ensure().expect("ensure store tree");
        let root = home.join("repo");
        ...
        assert!(
            response.results.iter().any(|r| {
                r.path.ends_with("deploy.yaml")
                    && r.unit_kind == "config_section"
                    && r.name == "spec"
                    && r.language == "yaml"
            }),
            "expected a named yaml config_section hit: {:?}",
            response.results
        );
    }
